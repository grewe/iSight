...
public class MainActivity extends AppCompatActivity implements CvCameraViewListener2, OnItemSelectedListener
{
    ...
    Boolean isProcessing = false;   //if true drop frames
    ...
    //declare Goolge Cloud Vision objects
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //get IconMODe if doing TensorFlow with Icons
        this.ICON_MODE = this.getResources().getString(R.string.ICON_MODE);
        this.ICON_MODE = "BoundingBox";
        ...
        //create the overlay ImageView for potential use later in drawing
        overlay_ImageView = new ImageView(this.getApplicationContext());
        overlay_ImageView.setImageResource(R.drawable.person);

        //tells how BIG width and height the View should be
        overlay_ImageView.measure(View.MeasureSpec.makeMeasureSpec(256, View.MeasureSpec.EXACTLY), View.MeasureSpec.makeMeasureSpec(256, View.MeasureSpec.EXACTLY));

        overlay_ImageView.layout(0,  0, 256, 256);
    ...
    public void onCameraViewStarted(int width, int height) {
        ...
        //CODE to grap the ViewGroup for the Overlay associated with the overlaying the cameraLinearLayout (which only contains the JCameraView used by OpenCV)
        VGO =  ((ViewGroup) findViewById(R.id.cameraLinearLayout)).getOverlay();
        // add the already created overlay_ImageView and ImageView to display our overlayed image on top of the JCameraView used by OpenCV
    ...
    public Mat onCameraFrame(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
       ...
       /*
        ...
        if(this.menu_item_selected.equals("TensorFlow")) {   //Random
            ...
            if(isProcessing == true){  //skip frames if TensorFlow still running
                return imageMat;
            }
            else {
                isProcessing =false;
            ...
            if(runMode == "ICON"){
                ...
                //do the following on a DIFFERENT thread so can see return imageMat to see live video while it is processing in the background
                // create anonymous Runnable and pass to init the 2 things needed this Activity AND the bitmap for processing
                (new Runnable() {
                    ...
                    public void run() {
                        ...
                        List<Classifier.Recognition> recognitionList = parent_Activity.tensorFlowDetector.recognizeImage(bitmap);
                        ...
                        for (final Classifier.Recognition result : recognitionList) {
                            ...
                            IconUtil.iconifyReconitionResult(parent_Activity, parent_Activity.VGO, (ViewGroup) parent_Activity.findViewById(R.id.cameraLinearLayout), parent_Activity.overlay_ImageView, result, parent_Activity.ICON_MODE, true, parent_Activity.ICON_DISPLAY_TIME_MS);
                        }

                        isProcessing = false;   //set flag so that can process the next frame
                    }
                    public Runnable init(MainActivity parent_Activity, Bitmap bitmap){
                        this.parent_Activity = parent_Activity;
                        this.bitmap = bitmap;
                        return(this);
                    }
                }).init(this, bitmap).run();
